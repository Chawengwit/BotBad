...
### ปิดโหมดฟังเมื่อ tool ทำงานจบ

`join_game` `leave_game` `mark_my_payment` = งานเสร็จในตัว ปิดโหมดฟัง
`propose_*` ยังรอกดปุ่มยืนยัน ส่วน `get_*` / `list_players` เป็นการถามข้อมูล ทั้งคู่ยังคุยต่อได้

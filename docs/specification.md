...
กติกาเดียวคือ **ปิดเมื่อดูแล้วไม่ได้คุยกับบอท** ไม่ใช่ปิดเมื่อทำงานเสร็จ
...
ผลข้างเคียงที่ยอมรับ: ปุ่มที่บอทเคยส่งไปแล้วยังค้างอยู่ในแชทและกดได้เสมอ
Postback `join` / `leave` / `list` / `bill_*` จึงต้องรองรับต่อไป แม้บอทจะไม่ส่งปุ่มพวกนี้ออกไปใหม่แล้ว
...
แต่ตอน **ถอนชื่อ** ต้องตอบว่าไม่รู้จัก ห้ามสร้าง
...
Bot ส่งรายชื่อ **ข้อความเดียว ไม่มีปุ่ม และไม่ส่งการ์ดรอบตีตามมาอีกใบ**
...
[ จำนวนคอร์ท ] [ จำนวนคน ] [ วันที่ ] [ เวลา ]
[ ระยะเวลา ] [ ชื่อคอร์ท ] [ แผนที่ ] [ พร้อมเพย์ ]
```
...
| `คิดเงิน <ชื่อบิล>` | บิลลอย ๆ ที่ไม่ผูกกับรอบตี |
...
get_open_game
list_players
join_game              รับ names[] เพื่อลงชื่อแทนหรือพาแขกมา
leave_game             รับ names[]
propose_create_game
propose_edit_game
propose_cancel_game
propose_close_game
...
| ใช้อะไร | กับอะไร |
|---|---|
| **Flex Message** | การ์ดทุกใบ: รอบตี, รายชื่อ, บิล, ใครยังไม่จ่าย และการ์ดยืนยันทุกแบบ |
| Buttons template | คำถามของ wizard ที่มีตัวเลือกไม่เกิน 4 ปุ่ม |
| Quick Reply | คำถามที่มีตัวเลือกเยอะ (จำนวนคน, เมนูแก้ไข, ค่าใช้จ่าย) และเมนูคำสั่ง |
| Datetime picker | เลือกวันและเวลา |
| ข้อความธรรมดา | ผลลัพธ์สั้น ๆ (ลงชื่อ, ถอนชื่อ, บันทึกการจ่าย), ข้อความ error และคำถามที่ต้องพิมพ์ตอบ |
...
บอทแนบปุ่ม (buttons template หรือ quick reply) ได้ **3 กรณีเท่านั้น**
...
วันศุกร์  → แตะฐานข้อมูล + push กลุ่มที่มีเรื่องจะบอก
```
...
| เรื่อง | เงื่อนไข |
|---|---|
| รอบที่เปิดอยู่ | บอกวัน เวลา คอร์ท และจำนวนคนที่ลงแล้ว — ไม่ว่าจะตีวันไหน |
| รอบค้างเก่า | วันเล่นผ่านไปแล้วแต่ยังไม่ปิด เตือนให้ปิด |

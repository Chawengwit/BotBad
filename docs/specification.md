...
ปุ่มทุกปุ่มอยู่ท้ายการ์ด Flex (footer) เรียงแถวละ 2 ปุ่ม ปุ่มยืนยันพื้นเขียว ปุ่มอื่นพื้นเทา
...
- Flex โหลดฟอนต์เองไม่ได้ ไอคอนจึงเป็นไฟล์ PNG ขนาด 96×96 ที่ `public/icons/<สี>/<ชื่อ>.png`
  LINE โหลดจาก `https://bot-bad.vercel.app/icons/...` (HTTPS เท่านั้น ไฟล์ต้อง deploy แล้วถึงจะขึ้น)
- มี 4 สี: `white` (บนพื้นเขียว/แดง), `ink` (ปุ่มรอง), `accent` (ข้อมูลทั่วไป), `warn` (คำเตือน) เพราะสีติดมากับรูป เปลี่ยนทีหลังไม่ได้

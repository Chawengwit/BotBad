...
describe("parseCommand", () => {
  ...
  /**
   * ส่วนเติมท้ายที่พูดถึงวันหรือรอบ ไม่ใช่รายชื่อคน (PRP multi-open-rounds §15)
   * ทดสอบกับ Gemini จริง 2026-09-19: "ลงชื่อ รอบวันเสาร์" กดรอบเสาร์แล้วได้แขกชื่อ "รอบวันเสาร์" แทนคนพิมพ์
   */
  it.each([
    ["ลงชื่อ รอบวันเสาร์", "ลงชื่อ"],
    ["ลงชื่อ วันเสาร์", "ลงชื่อ"],
    ["ลงชื่อ เสาร์นี้", "ลงชื่อ"],
    ["ลงชื่อ พรุ่งนี้", "ลงชื่อ"],
    ["ลงชื่อ รอบ 2 ทุ่ม", "ลงชื่อ"],
    ["ลงชื่อ กิ้ฟ รอบเสาร์", "ลงชื่อ"],
    ["ถอนชื่อ รอบพุธ", "ถอนชื่อ"],
    ["ถอนชื่อ พฤหัส", "ถอนชื่อ"],
  ])("%s พูดถึงรอบ ไม่ใช่รายชื่อคน", (input, command) => {
    expect(parseCommand(input)).toEqual({ command, args: "", aboutRound: true });
  });

  // ชื่อคนที่ขึ้นต้นด้วยคำเกี่ยวกับวันมีจริง ต้องยังเป็นรายชื่อ
  it.each([["ลงชื่อ วันชัย"], ["ลงชื่อ จันทร์เพ็ญ"], ["ถอนชื่อ กิ้ฟ วิท"]])("%s ยังเป็นรายชื่อคน", (input) => {
    expect(parseCommand(input)?.aboutRound).toBeUndefined();
  });
});
...
describe("ข้อความตอนลงชื่อแทนกัน", () => {
  ...
  // คนถามแค่ว่าชื่อนี้อยู่ในรายชื่อรอบนี้ไหม จะรู้จักชื่อหรือไม่ก็คำตอบเดียวกัน
  it("ถอนคนที่ไม่ได้อยู่ในรายชื่อ บอกตรง ๆ ว่าไม่อยู่ในรายชื่อรอบนี้ ไม่เงียบ", () => {
    const body = messageText(
      leftForNotice(
        "ฮก",
        { joinedCount: 0, people: [], skipped: [{ user: guest("กิ้ฟ"), reason: "not_joined" }] },
        ["louis"],
      ),
    );
    expect(body).toBe("ℹ️ กิ้ฟ, louis ไม่ได้อยู่ในรายชื่อรอบนี้");
  });
